...
if(!this.isScrolling){e.preventDefault();clearTimeout(this.interval);this.deltaX=this.deltaX/((!this.index&&this.deltaX>0||this.index==this.length-1&&this.deltaX<0)?(Math.abs(this.deltaX)/this.width+1):1);this.element.style.MozTransform=this.element.style.webkitTransform='translate3d('+(this.deltaX-this.index*this.width)+'px,0,0)';}},onTouchEnd:function(e){var isValidSlide=Number(new Date())-this.start.time<250&&Math.abs(this.deltaX)>20||Math.abs(this.deltaX)>this.width*this.items/2,isPastBounds=!this.index&&this.deltaX>0||this.index>=this.length-this.items&&this.deltaX<0;if(!this.isScrolling){var index=this.calcNewIndex(isValidSlide,isPastBounds,this.items);var trailing=this.length-index;if(trailing<this.items){index=this.calcNewIndex(isValidSlide,isPastBounds,trailing);}
index=Math.max(0,Math.min(this.length,index));this.slide(index,this.speed);}},calcNewIndex:function(isValidSlide,isPastBounds,itemCount){return this.index+(itemCount*(isValidSlide&&!isPastBounds?(this.deltaX<0?1:-1):0));}};